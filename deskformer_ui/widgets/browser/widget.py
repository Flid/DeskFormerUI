...
class DFBrowserWidget(DFWidgetBase):
    ...
    def start_cef(self):
        ...
        cef.Initialize(settings, self.switches)
        ...
        self.browser = cef.CreateBrowserSync(
            windowInfo, browserSettings, navigateUrl=self.start_url
        )
    ...
    def set_js_bindings(self):
        if not self._js_bindings:
            ...
            self._js_bindings.SetFunction(
                "__kivy__request_keyboard", self.request_keyboard
            )
            self._js_bindings.SetFunction(
                "__kivy__release_keyboard", self.release_keyboard
            )
    ...
    def devtools(self, *_):
        ...
        if "enable-begin-frame-scheduling" in self.switches:
            text = (
                "To enable DevTools you need to remove the\n"
    ...
    def on_touch_down(self, touch, *kwargs):
        ...
        if touch.is_double_tap:
            ...
            self.browser.SendMouseClickEvent(
                x, y, cef.MOUSEBUTTON_RIGHT, mouseUp=False, clickCount=1
            )
            self.browser.SendMouseClickEvent(
                x, y, cef.MOUSEBUTTON_RIGHT, mouseUp=True, clickCount=1
            )
        else:
            self.browser.SendMouseClickEvent(
                x, y, cef.MOUSEBUTTON_LEFT, mouseUp=False, clickCount=1
            )
    ...
    def on_touch_up(self, touch, *kwargs):
        ...
        # x, y = self.to_parent(x, y)
        self.browser.SendMouseClickEvent(
            x, y, cef.MOUSEBUTTON_LEFT, mouseUp=True, clickCount=1
        )
        self.is_mouse_down = False

        if self.is_drag:
            # print("on_touch_up=%s/%s" % (x,y))
            if self.is_drag_leave or not self.is_inside_web_view(x, y):
                ...
                print(
                    "[kivy_.py] ~~ current_drag_operation=%s"
                ...
                self.browser.DragTargetDrop(x, y)
                self.browser.DragSourceEndedAt(x, y, self.current_drag_operation)
                self.drag_ended()
    ...
    def on_touch_move(self, touch, *kwargs):
        ...
        self.browser.SendMouseMoveEvent(x, y, mouseLeave=False, modifiers=modifiers)
        if self.is_drag:
            print("on_touch_move=%s/%s" % (touch.x, y))
            if self.is_inside_web_view(x, y):
                if self.is_drag_leave:
                    print("[kivy_.py] ~~ DragTargetDragEnter")
                    self.browser.DragTargetDragEnter(
                        self.drag_data, x, y, cef.DRAG_OPERATION_EVERY
                    )
                    self.is_drag_leave = False
                print("[kivy_.py] ~~ DragTargetDragOver")
                self.browser.DragTargetDragOver(x, y, cef.DRAG_OPERATION_EVERY)
                self.update_drag_icon(x, y)
            else:
        ...
        if self.is_drag:
            if self.drag_icon:
                ...
                abuffer = image.GetAsBitmap(
                    1.0, cef.CEF_COLOR_TYPE_BGRA_8888, cef.CEF_ALPHA_TYPE_PREMULTIPLIED
                )
                # noinspection PyArgumentList
                texture = Texture.create(size=(width, height))
                texture.blit_buffer(abuffer, colorfmt="bgra", bufferfmt="ubyte")
                texture.flip_vertical()
                self.drag_icon = Rectangle(
                    texture=texture,
...
class ClientHandler:
    ...
    def _fix_select_boxes(self, frame):
        ...
        import deskformer_ui

        print("[kivy_.py] _fix_select_boxes()")
        resources_dir = os.path.join(
            os.path.dirname(deskformer_ui.__file__), "static", "kivy-select-boxes-fix"
        )
        log.warning(resources_dir)
        if not os.path.exists(resources_dir):
            print(
                "[kivy_.py] The kivy-select-boxes directory does not exist, "
        ...
        frame.ExecuteJavascript(
            jsCode, "kivy_.py > ClientHandler > OnLoadStart > _fix_select_boxes()"
        )
    ...
    def OnLoadStart(self, browser, frame, **_):
        ...
        if browserWidget and browserWidget.keyboard_mode == "local":
            print(
                "[kivy_.py] OnLoadStart(): injecting focus listeners for"
                " text controls"
            )
    ...
    def OnLoadingStateChange(self, is_loading, **_):
        ...
        if self.load_start_time:
            print(
                "[kivy_.py] OnLoadingStateChange: load time = {time}".format(
    ...
    def OnPaint(self, element_type, paint_buffer, **_):
        ...
        kivy_window = self.browserWidget.get_root_window()

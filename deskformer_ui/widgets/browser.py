...
class DFBrowserWidget(DFWidgetBase):
    ...
    headerHeight = NumericProperty(0)
    browserHeight = NumericProperty(800)
    browserWidth = NumericProperty(600)
    # Mouse wheel fudge to enhance scrolling
    scrollEnhance = NumericProperty(40)
    # desired frame rate
    frameRate = NumericProperty(100)

    # Define default background colour (black in this case)
    backgroundColour = ObjectProperty(sdl2.SDL_Color(255, 0, 0))
    ...
    def _run(self):
        ...
        browser_settings = {
            ...
            "windowless_frame_rate": self.frameRate
        }
        ...
        #window_id = EventLoop.window._win.get_window_info().window
        
        sleep(2)
        for _ in range(3):
            window_id = EventLoop.window._win.get_window_id()
            log.warning(window_id)
            window = sdl2.video.SDL_GetWindowFromID(window_id)
            #self._renderer = sdl2.render.SDL_GetRenderer(window)
        ...
        # log.info("Using hardware rendering")
        ...
        self._renderHandler = RenderHandler(self._renderer, self.width, self.height - self.headerHeight)
        ...
        while running:
            ...
            for event in events:
                ...
                if event.type == sdl2.SDL_MOUSEBUTTONDOWN:
                    if event.button.button == sdl2.SDL_BUTTON_LEFT:
                        if event.button.y > self.headerHeight:
                            log.info(
                            ...
                            self._browser.SendMouseClickEvent(
                                event.button.x,
                                event.button.y - self.headerHeight,
                    ...
                    if event.button.button == sdl2.SDL_BUTTON_LEFT:
                        if event.button.y > self.headerHeight:
                            log.info("SDL2 MOUSEBUTTONUP event (left button)")
                            # Mouse click triggered in browser region
                            self._browser.SendMouseClickEvent(
                                event.button.x,
                                event.button.y - self.headerHeight,
                    ...
                    if event.motion.y > self.headerHeight:
                        # Mouse move triggered in browser region
                        self._browser.SendMouseMoveEvent(event.motion.x,
                                                event.motion.y - self.headerHeight,
                                                False)
                    ...
                    x = event.wheel.x
                    if x < 0:
                        x -= self.scrollEnhance
                    else:
                        x += self.scrollEnhance
                    y = event.wheel.y
                    if y < 0:
                        y -= self.scrollEnhance
                    else:
                        y += self.scrollEnhance
                    self._browser.SendMouseWheelEvent(0, 0, x, y)
                elif event.type == sdl2.SDL_TEXTINPUT:
                    # Handle text events to get actual characters typed rather
                    # than the key pressed.
                    log.info("SDL2 TEXTINPUT event: %s" % event.text.text)
                    ...
                    self._browser.SendKeyEvent(key_event)
                    key_event = {
            ...
            sdl2.SDL_SetRenderDrawColor(
                self._renderer,
                self.backgroundColour.r,
                self.backgroundColour.g,
                self.backgroundColour.b,
                255
            )
            ...
            sdl2.SDL_RenderCopy(
                ...
                None,
                sdl2.SDL_Rect(0, self.headerHeight, self.browserWidth, self.browserHeight)
            )
            ...
            if sdl2.timer.SDL_GetTicks() - fpsTime > 1000:
                ...
                resetFpsTime = True
            # regulate frame rate
            if sdl2.timer.SDL_GetTicks() - startTime < 1000.0 / self.frameRate:
                sdl2.timer.SDL_Delay(
                    (1000 // self.frameRate) - (sdl2.timer.SDL_GetTicks() - startTime)
                )
